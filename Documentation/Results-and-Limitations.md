...
Two modelling choices follow from the pipeline's design: `mahalanobis-solve` is dropped (bit-identical to `mahalanobis`; see [Integrity of the numerical variants](#integrity-of-the-numerical-variants) below), and `mahalanobis-pinv` is kept and drawn dashed (it differs only in the covariance estimator, so it shows the estimator's effect on accuracy). The band in each panel is the seed-to-seed noise floor, so a batch-size trend is meaningful only where it rises above that band; a dotted line marks chance (0.5).
...
- **`mahalanobis-solve` is redundant in practice.** Being bit-identical to `mahalanobis` on these features (see [Integrity of the numerical variants](#integrity-of-the-numerical-variants)), it is kept only as a runnable, verifiable metric (`verify_maha_solve.py`) so the equivalence can be checked rather than assumed.
